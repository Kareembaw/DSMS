...
if (isset($_POST['join']) && !empty($_POST['join'])) {
  foreach ($_POST['join'] as $row_join) {
    $get_data_ligne_table = $conn->query("select instructorID from timetable where timetableID='" . $row_join . "'");
    $get_instructorID = $get_data_ligne_table->fetch();
    $reqexcut_insert = "INSERT INTO attandence (timetableID , status,instructorID,studentID) values(?,?,?,?)";
    $reqexcut_insert = $conn->prepare($reqexcut_insert);
    $reqexcut_insert->execute(array($row_join, 'test satuts', $get_instructorID['instructorID'], $SID));
  }
}
    ...
    <div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
        ...
        <main class="mdl-layout__content">
          ...
          ?><p><?= $error ?></p><?php
                    ...
                    <br>
                    <h3 style="margin-left: 39%;">CHOOSE TIMETABLE</h3>

                    <form method="POST">
                        <table style="margin-left: 38%;">
                            <tr>
                                <td class="mdl-data-table__cell--non-numeric">Insert Timetable
                                    ID:&nbsp;&nbsp;&nbsp;<input type="number" name="timetableID"></td>
                                <td class="mdl-data-table__cell--non-numeric"><button type="submit"
                                        name="joinClass">Join Class</button></td>
                            </tr>
                            <tr>
                ...
                ?>
                            <tr>
                                ...
                                <td class="mdl-data-table__cell--non-numeric"><input type="checkbox"
                                        value="<?= $TT['timetableID'] ?>" id="join" name="join[]"></td>
                            </tr>
                            <?php } ?>
                        </tbody>
                    </table>
